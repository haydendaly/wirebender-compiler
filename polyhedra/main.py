...
def dfs(current_path, visited, vertices, faces, path_length):
    ...
    for i in range(len(vertices)):
        if not visited[i] and is_connected(current_path[-1], i, faces):
            visited[i] = True
            new_path, new_path_length = dfs(
                current_path + [i],
                visited,
                vertices,
                faces,
                path_length
                + distance_between_points(vertices[current_path[-1]], vertices[i]),
            )
...
def traveling_salesman(vertices):
    ...
    for path in all_paths:
        cost = sum(
            distance_matrix[path[i], path[i + 1]] for i in range(num_vertices - 1)
        )
        if cost < min_cost:
            min_cost = cost
            min_path = path
...
def polyhedra_to_wirebender(vertices, faces, path):
    ...
    for i in range(len(path) - 1):
        ...
        angle = np.arccos(
            np.dot(start_vertex, end_vertex)
            / (np.linalg.norm(start_vertex) * np.linalg.norm(end_vertex))
        )
...
vertices, faces = generate_polyhedra("dodecahedron")
